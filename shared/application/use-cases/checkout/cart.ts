...
export async function hydrateCart(
  ...
  const tenantConfig = await api.fetchTenantConfig(
    apiClient.config.tenantIdentifier
  );
  ...
  const pickStandardPrice = (
    ...
    if (
      variant?.priceFor?.price &&
      variant?.priceFor?.price < variant?.price!
    ) {
  ...
  const cart = await handleCartRequestPayload(
    validatePayload<CartPayload>(body, cartPayload),
    {
      ...
      selectPriceVariant: (
        ...
        const variant = selectedVariant?.priceVariants?.find(
          (price: ProductPriceVariant) =>
            price?.identifier === "sales" &&
            price?.currency?.toLowerCase() === currency.toLocaleLowerCase()
        );
        const standardPrice = pickStandardPrice(
          product,
          selectedVariant,
          currency
        );
...
export const alterCartItemsAndCartTotalsBasedOnSavings = (
  ...
  const alteredItems: CartItem[] = cartWrapper.cart.cart.items.map(
    (item: CartItem) => {
      const saving =
        savings[item.variant.sku]?.quantity > 0
          ? savings[item.variant.sku]
          : null;
      const savingAmount = saving?.amount || 0;
      const netAmount = item.price.net - savingAmount;
      const taxAmount =
        (netAmount * (item.product?.vatType?.percent || 0)) / 100;
      ...
      return {
        ...
        price: {
          ...
          taxAmount,
          discounts: item.price.discounts
            ? [...item.price.discounts, discount]
            : [discount],
        },
      };
    }
  ...
  if (wrapper?.extra?.voucher) {
    if (wrapper.extra.voucher.value.type === "absolute") {
      voucherDiscount = {
        ...
        percent:
          (wrapper.extra.voucher.value.number / wrapper.cart.total.net) * 100,
      };
    }
    if (wrapper.extra.voucher.value.type === "percent") {
      voucherDiscount = {
        amount:
          (wrapper.extra.voucher.value.number / 100) * wrapper.cart.total.net,
        percent: wrapper.extra.voucher.value.number,
      };
    }
  }
  const currentCartTaxPercentage =
    (wrapper.cart.total.taxAmount / wrapper.cart.total.net) * 100;

  wrapper.cart.total.net -= voucherDiscount?.amount;

  wrapper.cart.total.taxAmount = +(
    (wrapper.cart.total.net * currentCartTaxPercentage) /
    100
  ).toFixed(2);

  wrapper.cart.total.gross =
    wrapper.cart.total.net + wrapper.cart.total.taxAmount;
