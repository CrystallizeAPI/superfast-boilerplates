...
export const DiscountedPrice: React.FC<{
  ...
  const priceSize = {
    small: {
      ...
      percentage:
        "text-sm py-1 px-2 h-[26px] rounded-md bg-[#efefef] font-medium",
    },
    medium: {
      ...
      percentage:
        "text-sm py-1 px-2 h-[26px] rounded-md bg-[#efefef] font-bold",
    },
  };
  let {
      ...
      {discountPrice && discountPrice < defaultPrice ? (
        <div className="flex flex-wrap  flex-col">
          <div className={priceSize[size as keyof typeof priceSize].previous}>
            <CrystallizePrice currencyCode={currency.code}>
              {defaultPrice}
            </CrystallizePrice>
          </div>
          <div className="flex gap-2 items-center">
            ...
            <div
              className={priceSize[size as keyof typeof priceSize].percentage}
            >
              -{discountPercentage}%
            </div>
          </div>
          {marketPrice && marketPrice < defaultPrice && (
            <p
              className={`${
                priceSize[size as keyof typeof priceSize].percentage
              } w-fit mt-2`}
            >
              {_t("marketPriceLabel")}
            </p>
...
export const Price: React.FC<{ variant: ProductVariant; size?: string }> = ({
  variant,
  size = "medium",
}) => {
