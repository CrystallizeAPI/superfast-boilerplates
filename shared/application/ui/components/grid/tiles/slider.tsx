...
export const Slider: React.FC<TileViewComponentProps> = ({ tile, options }) => {
  ...
  return (
    <div
      className={`${
        isFullWidth ? "px-8 lg:px-10 max-w-[1650px] w-full mx-auto" : "w-full"
      }`}
    >
      <div className={`pb-10  pt-20 ${setInnerPadding()}`}>
        ...
        {description && (
          <p className={`embed-text ${colspan > 2 ? "w-2/4" : "w-5/5"}`}>
            {description}
          </p>
        )}
        {ctas &&
          ctas.map((cta) => (
            <button
              className="bg-ctaBlue px-8 py-4 rounded font-medium"
              key={cta.link}
            >
              {cta.link ? (
      ...
      <div className={setInnerPadding()}>
        {content.items && (
          <CrystallizeCarousel
            options={{
              perPage: 2,
              itemLength: content?.items.length,
            }}
          >
