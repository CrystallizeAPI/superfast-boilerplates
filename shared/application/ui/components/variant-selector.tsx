...
function reduceAttributes(variants: ProductVariant[]) {
  return variants.reduce(
    (acc: Record<string, string[]>, variant: ProductVariant) => {
...
function findMostSuitableVariant(
...
export const VariantSelector: React.FC<{
  ...
  function onAttributeSelect(key: string, value: string) {
    ...
    if (!variant) {
      variant = variants.find((variant) =>
        Object.keys(variant.attributes).some(
          (vkey) => vkey === key && variant.attributes[vkey] === value
        )
      );
    }
    ...
    if (!variant) {
      variant = variants.find((variant) =>
        Object.keys(variant.attributes).some((vkey) => vkey === key)
      );
    }
          ...
          return (
            <div key={key} className="border-[#dfdfdf]">
              ...
              <div className="flex mb-5 flex-nowrap md:flex-wrap gap-2 overflow-x-scroll py-2 px-1">
                {values.map((value) => {
                  const mostSuitableVariant = findMostSuitableVariant(
                    variants,
                    {
                      ...selectedVariant.attributes,
                      [key]: value,
